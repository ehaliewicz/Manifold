...
void visit_graph_with_pvs(s16* pvs_table, u16 src_sector, u16 sector, u16 x1, u16 x2, u32 cur_frame, uint8_t depth) {
    #ifdef DEBUG_PORTAL_CLIP
    KLog_U1("in sector: ", sector);
    #endif 
    if(depth >= MAX_DEPTH ) {    
    ...
    s16 index_to_wall_list_for_sector = pvs_table[sector];
    //KLog_U1("for sector: ", sector);
    //KLog_S1("index to wall list for sector: ", index_to_wall_list_for_sector);
    if(index_to_wall_list_for_sector == -1) { 
        return; // not visible
    }
    // if this sector has been visited 32 times, or is already being currently rendered, skip it
    ...
    u16* visible_wall_list_for_sector = &cur_portal_map->wall_pvs[index_to_wall_list_for_sector];

    u16 num_visible_walls = *visible_wall_list_for_sector++;
    //KLog_U1("num visible walls in sector: ", num_visible_walls);

    Vect2D_s16 transformed_vert_buffer[62];
    Vect2D_s16* transformed_vert_buffer_ptr = transformed_vert_buffer;
    load_and_transform_pvs_walls(num_visible_walls, 
                                  visible_wall_list_for_sector, 
                                  map->walls,
                                  map->vertexes, transformed_vert_buffer);
    //Vect2D_s16 prev_transformed_vert = *transformed_vert_buffer_ptr++;
    ...
    //u32 pvs_bitmap = pvs_table[sector];
    //u32 mask = 0b1;


    for(int idx = 0; idx < num_visible_walls; idx++) {

        //u16 i = *visible_wall_list_for_sector++;
        u16 wall_idx = *visible_wall_list_for_sector++;
    ...
    //for(s16 i = 0; i < num_walls; i++) {
        ...
        //Vect2D_s16 trans_v1 = transformed_vert_buffer_ptr[i<<1];
        //Vect2D_s16 trans_v2 = transformed_vert_buffer_ptr[(i<<1)+1];
        ...
        //Vect2D_s16 trans_v1 = prev_transformed_vert;
        //prev_transformed_vert = trans_v2;
        //KLog_U1("wall idx : ", wall_idx);

        u16 portal_idx = wall_idx - sector;
        //u16 portal_idx = (wall_idx - wall_offset) + portal_offset;
        //KLog_U1("portal idx : ", portal_idx);
        //u16 portal_idx = portal_offset+i;
        ...
        if(clipped == OFFSCREEN) {
            ...
            if(sector == src_sector && is_portal) {
                ...
                neighbor_ceil_height = get_sector_group_ceil_height(portal_sect_group);
                if (neighbor_ceil_height > floor_height && neighbor_floor_height < ceil_height) {
                    visit_graph_with_pvs(pvs_table, src_sector, portal_sector, window_min, window_max, cur_frame, depth+1);
                    //add_new_sector_to_queue(window_min, window_max, portal_sector);
                }
        ...
        if (x1 > window_max) {
            if(is_portal && clipped & LEFT_Z_CLIPPED && x2 >= window_min) {
                visit_graph_with_pvs(pvs_table, src_sector, portal_sector, window_min, endx, cur_frame, depth+1);
            } else {
                #ifdef DEBUG_PORTAL_CLIP 
                if(is_portal) {
                    KLog_S2("x1 is off right hand side for portal idx : ", i, " to sect ", portal_sector);
            ...
            if(is_portal && clipped & RIGHT_Z_CLIPPED && x1 < window_max) {
                visit_graph_with_pvs(pvs_table, src_sector, portal_sector, beginx, window_max, cur_frame, depth+1);
            } else {
        ...
        if (is_portal) {
            ...
            KLog_S2("recursing through portal in sector: ", sector, " with wall idx: ", i);
            #endif
                visit_graph_with_pvs(pvs_table, src_sector, portal_sector,
                            ((clipped & LEFT_Z_CLIPPED) ? window_min : beginx),
                            ((clipped & RIGHT_Z_CLIPPED) ? window_max : endx),
                            cur_frame, depth+1);
